    ...
    useEffect(() => {
      const fetchCoin = async () => {
        try {
          ...
          setCoin(data);
          setchartArrays(chart_data.prices)
   ...
   <Container maxW={'container.xl'} >
      ...
      <HStack p={'4'} wrap={'wrap'}>
        ...
        <RadioGroup value={currency} onChange={setCurrency}>
          <HStack spacing={'4'} color={'red.900'}>
        ...
        <CustomBar high={`${CurrencySymbol} ${coin.market_data.low_24h[currency]}`} low ={`${CurrencySymbol} ${coin.market_data.low_24h[currency]}`}/>
        <Box w={'full'} p={'4'}>
...
const Item =({value ,title})=>(
  <HStack w={'full'} my={'4'} justifyContent={'space-between'}>
    <Text fontFamily={'Bebas Naue'} letterSpacing={'widest'} >{title}</Text>
    <Text >{value}</Text>
  </HStack>
)
